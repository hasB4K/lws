...
function run_tests() {
    if [ -n "$SCHEDULER_PROVIDER" ]; then
        ...
        $GINKGO --junit-report=junit.xml --output-dir=$ARTIFACTS -v --keep-going --focus="leaderWorkerSet e2e gang scheduling tests" $CWD/test/e2e/...
    else
        # Run normal tests, skip gang scheduling tests
        $GINKGO --junit-report=junit.xml --output-dir=$ARTIFACTS -v --keep-going --skip="leaderWorkerSet e2e gang scheduling tests" $CWD/test/e2e/...
    fi
}
